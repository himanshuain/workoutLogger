...
export default function ProgressGraph({ 
  ...
  const stats = useMemo(() => {
    if (graphData.length < 2) {
      return { trend: 'neutral', change: 0, maxWeight: graphData[0]?.weight || 0 };
    }
  ...
  const trendColor = stats.trend === 'up' ? 'text-lift-primary' : stats.trend === 'down' ? 'text-red-400' : 'text-iron-400';
      ...
      <div className="bg-iron-900/50 rounded-2xl overflow-hidden">
        ...
        >
          <div className="w-12 h-12 rounded-xl bg-iron-800 flex items-center justify-center flex-shrink-0">
            <ExerciseIcon name={exerciseName} className="w-8 h-8" color="#6b7280" />
          </div>
          <div className="flex-1 text-left min-w-0">
            ...
            <div className="flex items-center gap-2 mt-0.5">
              <span className={`flex items-center gap-1 text-sm ${trendColor}`}>
                ...
                {stats.change > 0 ? `+${stats.change}${unit}` : `${stats.change}${unit}`}
              </span>
              <span className="text-iron-600 text-sm">·</span>
              <span className="text-iron-500 text-sm">Max: {stats.maxWeight}{unit}</span>
            </div>
          </div>
          {isExpanded ? (
          ...
          <div className="px-4 pb-4">
            ...
            {graphData.length > 0 && (
              <div className="flex justify-between text-xs text-iron-500 mt-2">
                ...
                <span>Current: {stats.currentWeight}{unit}</span>
              </div>
            )}
          </div>
        )}
    ...
    <div className="bg-iron-900/50 rounded-2xl p-4">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-xl bg-iron-800 flex items-center justify-center">
            <ExerciseIcon name={exerciseName} className="w-8 h-8" color="#22c55e" />
          </div>
          <div>
            <h3 className="text-iron-100 font-semibold">{exerciseName}</h3>
            <p className="text-iron-500 text-sm capitalize">{exerciseCategory}</p>
          </div>
        </div>
        <div className={`flex items-center gap-1 px-3 py-1.5 rounded-lg ${
          stats.trend === 'up' ? 'bg-lift-primary/20' : 
          stats.trend === 'down' ? 'bg-red-500/20' : 'bg-iron-800'
        }`}>
      ...
      <div className="grid grid-cols-3 gap-3 mt-4">
        <div className="text-center p-2 bg-iron-800/50 rounded-xl">
          ...
          <p className="text-iron-100 font-bold">{stats.currentWeight || 0}{unit}</p>
        </div>
        <div className="text-center p-2 bg-iron-800/50 rounded-xl">
          <p className="text-iron-500 text-xs">Max</p>
          <p className="text-lift-primary font-bold">{stats.maxWeight || 0}{unit}</p>
        </div>
        <div className="text-center p-2 bg-iron-800/50 rounded-xl">
